...
import {
  client as Client,
  signMessageAndGetAddress,
} from "../../utils/blockchain/metamask-connect";
...
const ConnectMetamask = ({ setShowModal }) => {
  const {
    setMetamaskAccount,
    setAccount,
    chromia_account,
  ...
  const connect = async () => {
    ...
    try {
      ...
      const accountsEth = await window.ethereum.request({
        method: "eth_requestAccounts",
      });

      // returns the corresponding chromia account id for the metamask account
      let accounts = await queryParticipantByID(
        accountsEth[0].replace("0x", ""),
        client
      );
      if (!!accounts.length) {
        console.log("accounts", accounts);
        console.log(accounts[0]?.toString("hex"));
        //if account exists, getSession login   user and returning the session
        await getSession(accounts[0]?.toString("hex"));
      ...
      accounts = await queryParticipantByID(
        accountsEth[0].replace("0x", ""),
        client
      );

      let session = await getSession(accounts[0]?.toString("hex"));
      ...
      await session.call(op("initialize_ido", accounts[0]?.toString("hex"), accounts[0]?.toString("hex"), 3000));
      console.log("Done!!");
    } catch (err) {
      console.log(err);
  ...
  return (
    <>
      <div className="d-flex align-items-center menu-mob">
        ...
        {chromia_account == null && (
          <div className="m-0" onClick={connect}>
            <div
              className="btn btn-flex bg-body btn-color-gray-700 btn-active-color-primary fw-bold"
              data-kt-menu-trigger="click"
              data-kt-menu-placement="bottom-end"
